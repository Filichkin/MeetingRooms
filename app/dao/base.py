...
from sqlalchemy import (
    ...
    delete as sqlalchemy_delete, func
)
...
class BaseDAO(Generic[T]):
    ...
    async def find_one_or_none(self, filters: BaseModel):
        ...
        logger.info(
            f'Поиск одной записи {self.model.__name__} '
            'по фильтрам: {filter_dict}'
            )
        try:
            ...
            log_message = f'Запись {"найдена" if record else "не найдена"} '
    ...
    async def find_all(self, filters: BaseModel | None = None):
        ...
        try:
            ...
            logger.error(
                f'Ошибка при поиске всех записей по фильтрам '
                f'{filter_dict}: {e}'
                )
            raise
    ...
    async def add(self, values: BaseModel):
        ...
        logger.info(
            f'Добавление записи {self.model.__name__} '
